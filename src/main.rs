...
impl Display for QueensBoard {
    // Implement the Display trait for the custom type
    fn fmt(&self, f: &mut std::fmt::Formatter) -> Result<(), std::fmt::Error> {
        ...
        for row in self.iter() {
            ...
            for i in 0..self.0.len() {
                ...
                if i == *row as usize {
                    // If the column is the queen's column
                    rs.push_str("Q|"); // Add a Q to the string
                } else {
                    // If the column is not the queen's column
...
impl Genotype<u8> for QueensBoard {
    ...
    fn generate(size: &Self::ProblemSize) -> Self {
        ...
        for _i in 0..*size {
            // Iterate over the size of the problem
    ...
    fn fitness(&self) -> f64 {
        // Implement the fitness method for the custom type
        ...
        for _i in 0..diags_exceed as usize {
            // Iterate over the maximum diagonal index
            diagonals.push(-1); // Push a -1 to the diagonals vector
            inv_diags.push(-1); // Push a -1 to the inverse diagonals vector
        }

        for (row, queen) in self.0.iter().enumerate() {
            // Iterate over the rows and the queens
            ...
            if collision > -1 {
                // If there is a diagonal collision
                collisions[row] = true; // Set the collision to true
                collisions[collision as usize] = true; // Set the collision to true
            }
            ...
            if collision > -1 {
                // If there is a inverse diagonal collision
                collisions[row] = true; // Set the collision to true
                collisions[collision as usize] = true; // Set the collision to true
            }
...
fn main() {
    ...
    if n_queens <= 8 {
        // If the number of queens is less than or equal to 8
        population_size *= 2; // Double the population size
    }

    let (solutions, generation, progress, _population) =
        GeneticExecution::<u8, QueensBoard>::new() // Create a new genetic execution
    ...
    println!(
        ...
        generation,
        progress // Print the generation and progress
    );
